                ...
                <div class="col-xl-7 col-lg-5">
                  <div class="card-text">
                    ...
                    <div :class="user ? 'card__group-control--heart' : ''" class="card__group-control">
                      <div v-if="Number(productOnly.price) !== 0" class="price">{{Number(productOnly.price).toLocaleString('ru')}} $</div>
                      <div v-else class="price">Цена по запросу</div>
                          ...
                          <div class="row">
                            ...
                            <div v-if="productOnly.files.length" class="col-lg-6">
                              <template v-for="file of productOnly.files">
                                <a :key="file.id" :href="backURL+file.url" download target="_blank" class="download">{{file.name}}</a>
                                <br>
                                <br>
                              </template>
...
<script>
  ...
  export default {
    ...
    async fetch() {
      ...
      const { status, data } = await this.$axios.get(`/products?_where[catalog.slug]=${route.params.slugs}&_where[catalog_element.slug]=${route.params.slug}&slug=${route.params.id}`)
      if (status === 200) {
        const [item] = data
    ...
    computed:{
      ...
      chains() {
        ...
        return [
          ...
          {
            name: this.productOnly.catalog.name.toLowerCase(),
            url: '/catalog/' + this.$route.params.slugs
          },
          {
            name: this.productOnly.catalog_element.name.toLowerCase(),
            url: '/catalog/' + this.$route.params.slugs +'/'+ this.$route.params.slug
          },
          {
            name: this.productOnly.name.toLowerCase(),
            url: ''
          }
        ]
      },
